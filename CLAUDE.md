...
### UI 设计系统规范（高颜值现代温润风 + 手机电脑双端自适应）
- **设计哲学**：彻底告别生硬、难看、冰冷的黑客死黑极客控制台风！转向**高颜值、温润精致、舒适耐看的人性化现代 Web 产品设计**（现代 Linear / Apple 质感）。
- **微圆角体系（告别锋利刺手感）**：
  - 按钮与输入框：`rounded-lg (8px)`，触控按压温润饱满
  - 卡片与容器：`rounded-xl (12px)`，柔和视觉缓冲与包裹感
  - 弹窗浮层：`rounded-2xl (16px)`，沉浸式悬浮质感
  - 标签微标：`rounded-md (6px)`，紧凑精致
- **视觉色彩与材质（告别死黑黑白，注入现代生机）**：
  - 主色调：深邃但不压抑的高级深色系（微带冷灰底色 `#0b0f17`，卡片表面 `#151b26`，悬浮提亮 `#1e2638`），配发丝微光描边；
  - 状态点缀色（温润清爽）：翡翠绿（成功/开选）、琥珀金（倒计时/等待）、珊瑚红（满额/异常）、天青蓝（选中/核心高亮），信息层级分明且赏心悦目；
  - 字体排版：清晰现代字体栈，倒计时与名额统计保留等宽数字（`tabular-nums`），稳定无抖动。
- **双端自适应规范（Mobile First + Desktop 极致体验）**：
  - **移动端（手机浏览器）**：
    - 导航与操作：底部固定悬浮状态操作栏，单手拇指无障碍触达；
    - 触控热区：所有按钮与可点击元素最小高度 44px，防误触；
    - 列表流式排版：单列自适应课程卡片，严禁任何横向溢出滚动；
    - 抽屉式详情：课程详情在手机端自动以底部上滑抽屉（Drawer/Sheet）呈现，随手下滑即可关闭。
  - **电脑端（PC 宽屏桌面）**：
    - Bento 响应式网格排版，巨幕倒计时与三门目标课程清晰横向平铺，信息一目了然。
