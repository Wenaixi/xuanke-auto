# R133 前端只读审查报告（web/，React 18 + Vite + TS + Radix UI + Tailwind）

审查基线：commit `975dc2b`（R132 归档提交「LOW-132-01 时钟失败退避时间基统一对齐钟」，进度 133/256）。`git log --oneline 975dc2b..HEAD -- web/` 空输出、`git diff --stat 975dc2b HEAD -- web/` 空输出、HEAD 即 975dc2b——**R132 归档后 web/ 零提交、零产品改动**。f0f9bfc（P-1 倒计时 memo）仍为最近唯一 web 产品改动，已由 perf-countdown-guard 长期守卫。全链路只读，唯一写入为本报告。

## 必查项结论

### 1. M-1 第六十九轮闭合 — 通过（零漂移）
...
- **断言脚本实测**：`node --import jiti/register scripts/target-guard-check.ts`（web/ 目录）**18 条断言全绿、exit 0**（8 条 shouldDeferSave + 5 条 selectedHasStalePublish + 5 条 cleanStaleSelected，含"首帧未到 + 已回显标志 → 仍推迟"、"无变更 → 返回原引用"），输出与 R130/R131/R132 逐条一致。

### 2. OBSERVE-93-01 残余面第二十三轮 — 通过（零增零减）

`grep -rn "<button" web/src/` 全仓实证：**17 处 raw `<button`、3 文件**（Admin.tsx 13 / Dashboard.tsx 2 / Login.tsx 2），与 R132 基线完全一致，零增零减。明细行号逐一比对：
...
- Admin 651/661 引擎二选一按钮仍为 raw `<button>` 载体、强 active 姿态，`ui/Button` 收敛候选维持（R132 同口径，纯样式统一候选、无正确性/无障碍缺口）。

### 3. F93-01 第四十一轮 — 通过（双空实证，对比基准=R132 归档 commit 975dc2b）

- `git log --oneline 975dc2b..HEAD -- web/`：**空输出**。
- `git diff --stat 975dc2b HEAD -- web/`：**空输出**。
- 975dc2b 经 `git log -1 --format="%h %s"` 实证为 R132 归档提交（"fix(scheduler): LOW-132-01 时钟失败退避时间基统一对齐钟……"，前端零改动）。
- `git status --short` 工作树全净（报告写入前）；npm run build 产物落 `backend/web/dist/`（web/.gitignore 确认 dist 忽略）未污染工作树。

### 4. OBSERVE-116-01 跟踪项第十七轮维持 — 通过（注释如实口径，五路轮询契约零漂移）
...
三处 Esc 均带在飞守卫（退选/激活/删除进行中不响应防误关），autoFocus 均落在最安全默认项。行号与 R132 完全一致（R132 后零提交），逐字段语义零漂移。三处注释均含「补对话语义」及 Esc 补全的诚实口径，未僭越承诺。

### 6. R125 候选复核 — 维持成立（R125 首立、R126-R133 连续维持）
...
- **本轮是否立条**：维持候选记录（与 R125-R132 同口径），不立条、不做实现。收敛价值仅在 tick 间隔内省一次约 1255 行函数体重渲染；若落地需同步扩展 perf-countdown-guard 覆盖 Select 与 Select.tsx:204-207 注释口径升级，属改一增二，收益低于成本（P-1 只承诺 Dashboard）。
...
### 7. 新契约角度（自选纵深，R130-132 已走在飞守卫组件级/弹窗级分界、弹窗 Esc、保存链防抖/flush、激活码复制可访问性，本轮转向手动操作在飞守卫全路径补核 + 折叠 extrasOpen 键盘结构 + Admin 激活码复制降级链复核）
...
**Dashboard 折叠 extrasOpen 结构与键盘路径（无缺口）**：CollapseSection（:50-88）按钮带 `aria-expanded={open}`/`aria-controls={id}`（:61-62），id 来自 useId（:69）——多实例化（extras 折叠段 :430 + 每日期组 :553）时唯一 id 零冲突，注释自证由硬编码 "collapse-body" 悬空 id 修复而来；`{open && <div id={id}>}`（:86）条件渲染、展开后 aria-controls 目标落位，收起时合法悬空（ARIA 契约允许）。extrasOpen 布尔态由 `onToggle={() => setExtrasOpen((o) => !o)}` 切换；extrasMs 空（begin_times 全被主时间吸收）时段落整体条件隐藏，extrasOpen 残留 true 无副作用（无渲染位消费 false 态）。折叠钮为键盘可聚焦原生 `<button>`，零额外成本可访问性。相对时间摘要 relativeCountdown 由主矩阵每秒 tick 的整页重渲染驱动（:229 注释）渲染期直算 Date.now()（:230 nowMs），最坏 1 秒陈旧、绝不另建定时器——折叠行不重复订阅 tick。无缺口。

**Admin 激活码复制降级链复核（无缺口）**：`navigator.clipboard.writeText` 非安全上下文/内网明文部署时整体不可用（:69-71 注释）→ 首判 `!navigator.clipboard?.writeText` 即抛错入 catch（:75）；catch 内 execCommand 兜底三件套——textarea `readOnly:true`（:91）防可编辑区干扰选中/复制、`position:fixed + opacity:0`（:85-87）内存驻留视觉隐藏不入可读视觉链、成功后 `document.body.removeChild`（:94）DOM 清理；仍失败才 toast 提示并展示完整激活码供抄录。复制定时器 copyTimer 1500ms 复位（:79-81）防 Check 图标残留。降级全链（API→execCommand→抄录展示）有明确出口无死路。无缺口。
...
| npm run build（tsc -b + vite） | **通过**（1948 modules transformed，built in 1.85s，产物落 backend/web/dist，exit 0） |
| `git log 975dc2b..HEAD -- web/` | 空（F93-01 双空实证之一，R132 归档 commit 起零提交） |
| `git diff --stat 975dc2b HEAD -- web/` | 空（F93-01 双空实证之二） |
...
| 轮次标签扫描 | 零命中 |
| 975dc2b 归档确认 | `git log -1` 实证 = R132 归档提交（进度 133/256） |

## 分级发现

- **必查项**：7/7 全部通过（M-1 第六十九轮 / OBSERVE-93-01 第二十三轮 / F93-01 第四十一轮 / OBSERVE-116-01 第十七轮 / OBSERVE-115-01 弹窗族 / R125 候选复核 / 新契约角度在飞守卫全路径 + 折叠 extrasOpen + 激活码复制降级链复核）。
...
R132 归档 commit 975dc2b 即当前 HEAD——web/ 零提交、零产品改动，七项必查全部在位且零漂移，四守卫全绿（18/3/6/5），构建通过（1948 modules），工作树全净。本轮为**延续性确认轮**：所有历史锚点（M-1/OBSERVE-93-01/F93-01/OBSERVE-116-01/OBSERVE-115-01/R125 候选）持续闭合，无新增分级发现，无需 REQUEST。进度推进至 134/256。