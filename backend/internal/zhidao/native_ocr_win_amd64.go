...
//go:embed assets/onnxruntime_win_amd64.dll
var embeddedDll []byte

// NativeDdddOcrRecognizer 真正的单二进制内置 ddddocr 识别引擎（纯 Go + ONNX 原生嵌入）。
//
// 运行原理：
//  1. 模型（common_old.onnx，13MB）、字符集（charsets_old.json，56KB）和 ONNX Runtime
//     （onnxruntime_win_amd64.dll，16MB）均通过 //go:embed 原生编译进单个 exe 二进制内部；
//  2. 运行时若本地不存在，以 0.05 秒自动从内存释放到系统临时目录中；
...
func (r *NativeDdddOcrRecognizer) ensureInit() error {
	r.initMu.Do(func() {
		...
		dllPath := filepath.Join(dir, "onnxruntime_win_amd64.dll")
		...
		if err := dumpIfDiff(dllPath, embeddedDll); err != nil {
			r.err = fmt.Errorf("释出 onnxruntime_win_amd64.dll 失败: %w", err)
			return
		}
...
// NativeDdddOcrAvailable 检查是否支持原生内置识别（只要内嵌切片有数据，Windows amd64 环境即 100% 可用）。
