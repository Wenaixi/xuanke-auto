...
export default function Dashboard({ onLogout, onGoSelect }: Props) {
  ...
  const openTime = state?.open_time && state.open_time !== "0001-01-01T00:00:00Z"
    ? new Date(state.open_time)
    : null
    ...
    <div className="max-w-4xl mx-auto p-6 flex flex-col gap-6">
      <header className="flex items-center justify-between border-b pb-4">
        <div>
          ...
          <p className="text-xs text-[var(--fg-dim)] mt-1">选课窗口开启后自动并发报名，无需值守</p>
        </div>
      ...
      {/* 窗口状态 */}
      <section className="border p-4 flex flex-col gap-3">
        {stateLoading && <div className="text-sm text-[var(--fg-dim)]">状态加载中…</div>}
        {!stateLoading && stateErr && (
          <div className="text-sm text-[var(--danger)]">
            状态查询失败：登录可能已失效，请重新登录
          </div>
        )}
        {!stateLoading && !stateErr && (
          <>
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm">
                选课窗口：{state?.window_opened ? "已开放，正在抢课" : "未开放"}
              </span>
              {openTime && (
                <span className="text-sm text-[var(--fg-dim)] ml-auto">
                  开放时间 {openTime.toLocaleString("zh-CN", { hour12: false })}
                </span>
              )}
            </div>
            {openTime && !state?.window_opened && (
              <div className="text-sm text-[var(--warn)]">
                距离开放还有 {countdown(openTime.toISOString())}
              </div>
            )}
          </>
        )}
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {courses.map((c) => {
          ...
          return (
            <div key={c.class_id} className="border p-4 flex flex-col gap-2">
              <div className="text-xs text-[var(--fg-dim)]">发布 {c.publish_id}</div>
              <div className="font-medium">{c.course_name || ("课程 " + c.class_id)}</div>
              <div className="text-sm flex items-center gap-2" style={{ color: st.color }}>
                <span className="inline-block w-1.5 h-1.5" style={{ background: st.color }} />
                {st.label}
              </div>
        ...
        {courses.length === 0 && (
          <div className="border p-4 text-sm text-[var(--fg-dim)] col-span-full flex flex-col gap-3 items-center">
            <span>尚未设置目标课程</span>
            <Button onClick={onGoSelect}>去选择课程</Button>
          </div>
        )}
      </section>
        ...
        {logs?.length ? (
          <ul className="flex flex-col gap-1 text-sm">
            {logs.map((l) => (
              <li
                key={l.id}
                className="flex gap-3 items-baseline border-b border-[var(--border)]/40 pb-1"
              >
                <span className="text-xs text-[var(--fg-dim)] whitespace-nowrap">{l.created_at}</span>
                <span style={{ color: l.is_ok ? "var(--ok)" : "var(--danger)" }}>
                  {l.is_ok ? "成功" : "失败"}
                </span>
                <span className="text-xs text-[var(--fg-dim)] whitespace-nowrap">{l.action}</span>
                <span className="flex-1 truncate" title={l.result}>{l.result}</span>
              </li>
            ))}
          </ul>
        ) : (
